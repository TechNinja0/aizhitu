...
已保存文件区分查看和编辑，所有者默认编辑，接收者按授权主动切换，使用页面会话及对象属性三方合并；协议、表结构、状态机及故障边界见 [多人协同技术方案](COLLABORATION.md)。前端已移除独占锁，以“编辑”按钮切换本页模式，服务端保留旧锁 API 兼容。
...
- 启动时把旧 `draft=1` 文件改为私有普通文件，不修改内容、版本、创建者、时间和回收站状态；列表仅保留全部文件/回收站。列表重命名与 `/api/documents/batch-trash` 必须没有在线协同页面或有效旧编辑锁；批量最多 100 份，`BEGIN IMMEDIATE` 中先全量验证再写入，任何失败整体回滚。恢复回到全部文件。
- 协同会话绑定账号、文档与页面 ID，随机 token 和 30 秒有效期保存在 SQLite；依据是否有待同步内容、前后台及页面数量，每 1/5/10 秒续约及读取变更。多个账号、同账号不同标签页都可同时编辑，最多 32 个页面。
- 内容提交在 `BEGIN IMMEDIATE` 中验证会话、权限、请求回执和基线，执行对象属性合并并原子落盘。有修改时约每秒自动同步，单页空闲降低到 5 秒、后台空闲 10 秒；仅内容/名称变化新增版本；断线保留在途请求 ID 与本地修改重试，同属性按服务端提交顺序处理。
- 返回前冻结标题与画布并完成最后同步；失败保留页面。恢复历史和编辑页删除验证当前会话、revision 且没有其他编辑页面在线；列表批量管理要求没有任何在线编辑页面。旧锁获取也会拒绝已有协同会话，兼容接口不能绕过保护。
- 适配器 `setReadOnly` 禁用图形编辑、原生菜单/快捷键和修改类桥接命令，仍允许读取快照、查看与导出。服务器写入校验是最终边界。
- 每份文档最多保留最近 50 个服务器版本，按 64 MiB XML 总容量缩减，至少保留最近两版。删除为软删除，恢复原图或历史内容均递增版本。备份应停止服务后复制整个数据目录。
...
分享范围决定谁能读取，`shareRole` / `document_members.role` 决定被授权者能否修改。新分享默认仅查看，指定成员逐人设置；已有分享迁移保留可编辑。服务端 `requireEdit` 统一校验协同加入/保存、兼容旧编辑锁及保存、历史恢复和共享 AI 请求，客户端只读按钮不是权限边界。

接收者（包括管理员打开他人文件）默认查看；查看状态不申请会话，以文档状态接口按 1/5/10 秒策略获取更新。编辑按钮只向有写权限者提供，进入时加载最新内容并加入会话，退出时先提交正在输入的文字和最终同步，成功才离开会话。查看者不影响恢复历史、删除或转移文件；未同步修改遇到降权会暂停，保留下载副本能力。“新建 / 打开”也使用与返回文件库相同的保存屏障，失败保留当前文件。
