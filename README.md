...
主页为文件库，新建空白稿或选择模板后，首次修改内容或名称才自动保存到“全部文件”；未修改直接返回不留下文件。也可主动保存未修改图稿，导入文件直接入库。已保存的图稿默认仅创建者与本机管理员可见。点击分享图标，选择“仅自己 / 指定成员 / 所有成员”并保存权限后才会开放访问。支持创建人显示、我拥有的筛选、图稿名称编辑、批量移入回收站和恢复。每份图稿有独立地址，支持固定账号密码、记住登录、单人编辑锁、自动保存、其他页面自动更新和服务器版本。
...
支持通过本机 CLI 的网页内 AI 对话；提供固定账号和局域网共享编辑；暂不提供云同步、多人同时实时编辑或 MCP 外部实时画布控制。它是需要 Node.js 本地服务的应用，**不能直接部署为 GitHub Pages 静态网站**。
...
当前支持单画布和局域网共享工作区：多人查看、一人编辑、保存后自动更新。多页、复杂 UML/BPMN、多人同时编辑合并和公网部署不在本版范围。当前文件保存与权限实测见 [v1.6 验证报告](docs/testing/V1_6_AUTOSAVE_REPORT.md)，账号管理见 [v1.5 验证报告](docs/testing/V1_5_ACCOUNTS_REPORT.md)。
