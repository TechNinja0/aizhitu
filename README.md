...
主页现在是共享文件库。每份图稿有独立地址，支持首次填写用户名、单人编辑锁、自动保存、其他页面自动更新、服务器版本和回收站。

```sh
node bin/zhitu.mjs --lan-host 192.168.1.10 --port 4317 --editor-port 4318
```

将地址替换为服务电脑的内网 IP。允许两个端口访问，成员首次填写用户名即可加入，同一浏览器自动记住身份，无需密码或访问码。服务电脑使用 `http://127.0.0.1:4317/` 进入管理界面。数据默认保存在服务器 `~/.ai-zhitu/workspace.sqlite`。AI 使用服务器管理员配置的客户端和额度。详见 **[局域网部署、共享操作与备份](docs/LAN_WORKSPACE.md)**。

### 先用示例体验

1. 在文件库进入 **本地临时画布**，点击 **本地示例 → 系统架构** 或 **流程与条件分支**。
2. 双击节点修改文字，拖动节点和连接线，调整样式。
3. 点击 **保存副本**，下载可继续编辑的 `.drawio` 文件。
...
支持通过本机 CLI 的网页内 AI 对话；提供浏览器身份和局域网共享编辑；暂不提供云同步、多人同时实时编辑或 MCP 外部实时画布控制。它是需要 Node.js 本地服务的应用，**不能直接部署为 GitHub Pages 静态网站**。
...
当前支持单画布和局域网共享工作区：多人查看、一人编辑、保存后自动更新。多页、复杂 UML/BPMN、多人同时编辑合并和公网部署不在本版范围。共享功能的实测结果见 [v1.2 验证报告](docs/testing/V1_2_SHARED_REPORT.md)。
